    ...
    <div className="flex gap-1 ">
      {recentSearch.map((search) => (
        <div key={search} className="relative">
          <p key={search} className=" text-sm text-gray-500 border border-gray-300 rounded-xl pl-4 pr-6 py-1 hover:bg-gray-100 cursor-pointer">{search}</p>
          <X className="absolute right-1.5 top-1.5 w-4 h-4 text-gray-400 hover:text-gray-700 cursor-pointer" />
        </div>
      ))}
    </div>
      ...
      <div className="flex flex-col flex-wrap gap-1 w-full h-60">
        {popularSearch.map((search, idx) => (
          <div key={search} className="flex items-center gap-2 hover:bg-gray-100 p-1 rounded-md cursor-pointer w-1/2">
            <p className="text-sm text-gray-500 w-8 h-8 flex items-center justify-center rounded-md bg-gray-100">{idx + 1}</p>
            <p className="text-sm text-gray-500 text-nowrap">{search}</p>
          </div>
        ))}
      </div>
    </div>
